...
void PressureManager::update_pressures(GearboxGear current_gear, GearChange change_state) {
    ...
    if (CHECK_MODE_BIT_ENABLED(DEVICE_MODE_SLAVE)) {
        ...
        if (this->shift_sol_en) {
            ...
            sol_spc->set_current_target(this->pressure_pwm_map->get_value(this->corrected_spc_pressure, sensor_data->atf_temp+50.0, 0u));
        } else {
        ...
        sol_mpc->set_current_target(this->pressure_pwm_map->get_value(this->corrected_mpc_pressure, sensor_data->atf_temp+50.0, 1u));
